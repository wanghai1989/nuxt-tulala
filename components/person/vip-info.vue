	...
	<div class="laymshade hand" v-if="show_withdraw">
			...
			<div class="about-tit">
				...
				<span v-else-if="type==2">图啦币总数 <strong> {{platform_currency}}</strong> <a href="javascript:void(0)" class="btn-withdraw" @click="showWithdraw()" >提现</a></span>
				 <span v-else><span v-if="type==3">到期时间 <strong>2021-11-30</strong></span> <router-link class="btn-openvip" to="/user/pay-vip" >{{personInfo.is_vip?'续期VIP':'开通VIP'}}</router-link></span>
			</div>
